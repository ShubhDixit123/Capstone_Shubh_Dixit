...
WITH flattened_suppliers AS (

    SELECT

        supplier_item.value AS supplier_record,
...
processed_suppliers AS (

    SELECT

        /* Natural Key */
        TRIM(
            supplier_record:supplier_id::STRING
        ) AS supplier_id,
